...
<head>
    ...
    <script>
        // Funzione per verificare la chiave di attivazione
        function checkActivationKey() {
            fetch('key.php')
                .then(response => {
                    if (response.redirected) {
                        // Se la risposta è un reindirizzamento, reindirizza l'utente
                        window.location.href = response.url;
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    // Puoi gestire eventuali errori qui, ad esempio:
                    window.location.href = 'key_error.php';
                });
        }

        // Lancia la verifica della chiave di attivazione al caricamento della pagina
        window.onload = checkActivationKey;
    </script>
</head>
