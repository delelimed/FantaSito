...
if (isset($_SESSION['id']) && isset($_SESSION['nome']) && $_SESSION['locked'] == 0 && $_SESSION['admin'] == 1){
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Gestisci Interleghe</h1>
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <h3 class="card-title">Elenco Interleghe</h3>
                        </div> <!-- /.card-header -->
                        <div class="card-body p-0">
                            <table class="table table-striped">
                                <thead>
                                <tr>
                                    <th>Nome Interlega</th>
                                    <th>Partecipanti</th>
                ...
                ORDER BY 
                    l.nome_interlega ASC"; // Aggiunto l'ordinamento alfabetico per nome_interlega
                                ...
                                if ($result && mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        echo '<tr>';
                                        echo '<td>' . htmlspecialchars($row['nome_interlega']) . '</td>';
                                        echo '<td>' . htmlspecialchars($row['partecipanti']) . '</td>';
                                        echo '<td style="width: 40px">0</td>'; // Puoi sostituire 0 con la logica per i punti
                                        echo '<td>';
                                        echo '<button class="btn btn-warning" onclick="modificainterLega(' . $row['id_interlega'] . ', \'' . addslashes($row['nome_interlega']) . '\', \'' . htmlspecialchars($row['partecipanti']) . '\', \'' . $row['partecipanti_ids'] . '\')">Modifica</button>';
                                        echo '<button class="btn btn-danger" onclick="eliminainterLega(' . $row['id_interlega'] . ')">Elimina</button>';
                                        echo '</td>';
                                        echo '</tr>';
                                    }
                                } else {
                                    echo '<tr><td colspan="4">Nessuna interlega trovata.</td></tr>';
                                }
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="modificainterLegaModal" tabindex="-1" role="dialog" aria-labelledby="modificainterLegaLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modificainterLegaLabel">Modifica Interlega</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <form id="modificainterLegaForm">
                                        <input type="hidden" id="idinterLega" value="">
                                        <div class="form-group">
                                            <label for="nomeinterLega">Nome Interlega</label>
                                            <input type="text" class="form-control" id="nomeinterLega" name="nome_interlega" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="partecipantiinterLega">Partecipanti</label>
                                            <select class="form-control select2" id="partecipantiinterLega" name="partecipanti[]" multiple="multiple" required>
                                                <?php
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn btn-primary" id="salvaModificheinterLega">Salva Modifiche</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal fade" id="aggiungiinterLegaModal" tabindex="-1" aria-labelledby="aggiungiinterLegaModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="aggiungiinterLegaModalLabel">Aggiungi Interlega</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <form id="formAggiungiinterLega">
                                        <div class="form-group">
                                            <label for="nomeinterLega">Nome Interlega</label>
                                            <input type="text" class="form-control" id="nomeinterLegaadd" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="partecipantiinterLega">Partecipanti</label>
                                            <select id="partecipantiinterLegaadd" class="form-control" multiple="multiple" style="width: 100%;" required>
                                                <?php
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Annulla</button>
                                    <button type="button" class="btn btn-primary" id="salvaNuovainterLega">Aggiungi Interlega</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#partecipantiinterLegaadd').select2({
                placeholder: 'Seleziona i partecipanti',
                width: '100%'
            });

            // Gestione del salvataggio della nuova lega
            $('#salvaNuovainterLega').on('click', function() {
                var nomeinterLega = $('#nomeinterLegaadd').val().trim(); // Rimuove spazi vuoti
                var partecipanti = $('#partecipantiinterLegaadd').val(); // Ottiene l'array di ID selezionati

                // Controlla se il nome della lega è vuoto o se non ci sono partecipanti selezionati
                if (!nomeinterLega) {
                    alert('Inserisci un nome per la interlega.');
                ...
                $.ajax({
                    url: '../req/gestione_interleghe/aggiungi_interlega.php', // Il file PHP per aggiungere la lega
                    method: 'POST',
                    data: {
                        nome_interlega: nomeinterLega,
                    ...
                    success: function(response) {
                        ...
                        if (data.success) {
                            ...
                            alert(data.message);
                            $('#aggiungiinterLegaModal').modal('hide');
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            console.log($('#partecipantiinterLega').length ? "Element exists" : "Element not found");
        });


        function modificainterLega(idinterLega, nomeinterLega, partecipanti, partecipantiIds) {
            // Popola i campi della modale con i dati della lega
            $('#idinterLega').val(idinterLega); // Inserisci l'id della lega nel campo nascosto
            $('#nomeinterLega').val(nomeinterLega);

            // Reset della select dei partecipanti
            $('#partecipantiinterLega').val(null).trigger('change');
            ...
            for (var i = 0; i < partecipantiIdsArray.length; i++) {
                $('#partecipantiinterLega').val(partecipantiIdsArray).trigger('change');
            }

            // Mostra la finestra modale
            $('#modificainterLegaModal').modal('show');
        }


        // Gestione del salvataggio delle modifiche
        $('#salvaModificheinterLega').on('click', function() {
            var nomeinterLega = $('#nomeinterLega').val();
            var partecipanti = $('#partecipantiinterLega').val();

            console.log("Nome Lega:", nomeinterLega);
            console.log("Partecipanti IDs:", partecipanti);

            // Chiudi la finestra modale
            $('#modificainterLegaModal').modal('hide');
        });

        // Gestione del salvataggio delle modifiche
        $('#salvaModificheinterLega').on('click', function() {
            var idinterLega = $('#idinterLega').val(); // ID della lega nascosto nella modale
            var nomeinterLega = $('#nomeinterLega').val();
            var partecipanti = $('#partecipantiinterLega').val(); // Ottiene l'array di ID selezionati

            // Invio dei dati tramite AJAX
            $.ajax({
                url: '../req/gestione_interleghe/salva_interlega.php',
                method: 'POST',
                data: {
                    id_interlega: idinterLega,
                    nome_interlega: nomeinterLega,
                    partecipanti: partecipanti // Array di ID dei partecipanti
                },
                success: function(response) {
    ...
    <script>
        function eliminainterLega(id) {
            if (confirm('Sei sicuro di voler eliminare questa interlega?')) {
                // Invia una richiesta AJAX per eliminare la lega
                const xhr = new XMLHttpRequest();
                xhr.open('POST', '../req/gestione_interleghe/elimina_interlega.php', true);
