        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    ...
                    // Query per recuperare tutti gli inserimenti
                    $query = "SELECT utente, data_inserimento, data_eseguito, eseguita FROM fs_recupero_pssw ORDER BY data_inserimento DESC";
                    $result = $conn->query($query);
                    ?>

                    <div class="container mt-5">
                        <h2>Inserimenti per Reset Password</h2>
                        <table class="table table-hover">
                            <thead>
                            <tr>
                                <th>Username</th>
                                <th>Data Inserimento</th>
                                <th>Data Ripristino</th>
                                <th>Azioni</th>
                            </tr>
                            </thead>
                            <tbody>
                            ...
                            if ($result && $result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    echo '<tr>';
                                    echo '<td>' . htmlspecialchars($row['utente']) . '</td>';

                                    // Formatta la data di inserimento
                                    $data_inserimento = date('d/m/Y H:i:s', strtotime($row['data_inserimento']));
                                    echo '<td>' . $data_inserimento . '</td>';

                                    // Formatta la data di ripristino
                                    $data_eseguito = $row['data_eseguito'] ? date('d/m/Y H:i:s', strtotime($row['data_eseguito'])) : 'N/A';
                                    echo '<td>' . $data_eseguito . '</td>';

                                    echo '<td>';
                                    // Controlla se "eseguito" è 1 e disabilita il pulsante se necessario
                                    if ($row['eseguita'] == 1) {
                                        echo '<button class="btn btn-secondary btn-sm" disabled>RESET!</button>'; // Pulsante disabilitato
                                    } else {
                                        echo '<button class="btn btn-danger btn-sm" onclick="resetUser(\'' . htmlspecialchars($row['utente']) . '\')">RESET!</button>';
                                    }
                                    echo '</td>';
                                    echo '</tr>';
                                }
                            } else {
                                echo '<tr><td colspan="4">Nessun inserimento trovato.</td></tr>';
                            }
                            ?>
                            </tbody>
                        </table>
                    ...
                    <script>
                        function resetUser(username) {
                            // Invia una richiesta POST a reset_password.php
                            fetch('../req/reset_password.php', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                },
                                body: JSON.stringify({ utente: username }),
                            })
                                ...
                                .then(data => {
                                    ...
                                    if (data.success) {
                                        ...
                                        // Puoi anche aggiornare la tabella o fare altre operazioni qui
                                    } else {
                                        alert('Errore: ' + data.message); // Mostra un messaggio di errore
                                    }
                                })
                                .catch((error) => {
